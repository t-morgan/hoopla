...
def movie_to_search_text(movie: dict) -> str:
    # genre is now a list of strings
    # cast is now a list of strings
    cast = movie.get("cast", [])
    if cast and isinstance(cast[0], dict):
        cast_names = [c.get("name", "") for c in cast if isinstance(c, dict) and "name" in c]
    else:
        cast_names = [str(c) for c in cast]
    parts = [
        movie.get("title", ""),
    ...
    return " ".join(p.strip() for p in parts if p and isinstance(p, str))
...
class InvertedIndex:
    ...
    def load(self) -> None:
        ...
        cache_files = [INDEX_PATH, DOCMAP_PATH, TERM_FREQUENCIES_PATH, DOC_LENGTHS_PATH]
        missing = [p for p in cache_files if not os.path.exists(p)]
        if missing:
            print(f"Index cache files missing: {missing}. Building index from scratch...")
            self.build()
            self.save()
        ...
        # Defensive: if any are still None, build and save again
        if any(x is None for x in [self.index, self.docmap, self.term_frequencies, self.doc_lengths]):
            print("Some index components failed to load. Rebuilding index...")
            self.build()
            self.save()
            self.index = load_pickle(INDEX_PATH)
            self.docmap = load_pickle(DOCMAP_PATH)
    ...
    def __get_doc_text(self, doc: dict) -> str:
        return movie_to_search_text(doc)
